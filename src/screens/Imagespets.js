...
function Imagespets() {
  ...
  const [message, setMessage] = useState("Please wait after submitting the query till we return the result");
  ...
  const checkoutHandler = () => {
    if (userInfo) navigate("/findcat");
    else {
      navigate(`/login?redirect=${"findcat"}`);
    }
  };
  ...
  const submitHandler = async (e) => {
    ...
    console.log(formData);
      ...
      try {
        const response = await axios.post("/api/users/cat-image/", formData, {
          headers: {
              ...
              >
                ...
                <Form.Group controlId="description">
                  <Form.Label style={{ marginTop: "10px", fontWeight: "bold" }}>
                    Tell us something about your Cat
                  </Form.Label>
                  <Form.Control
                    as="textarea"
                    row="3"
